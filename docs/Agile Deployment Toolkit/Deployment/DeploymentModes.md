...
When you are in DEVELOPMENT mode and you have a handful of developers a simple thing you can do is to use the cloud native firewall provided by your VPS provider to restrict access to your development webserver to only the IP addresses of your developers. This is a very easy way to protect your development environment in a robust way. If you want to protect your servers in a production environment then the authentication server technique is probably the way to go but you could also use a service like Cloudflare Zero Trust to restrict access but what I did with the authentication servers is to attempt to go to a zero trust model in a frugal way.
