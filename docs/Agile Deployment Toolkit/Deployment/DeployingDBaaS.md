...
There are two key settings to provision a managed database that need to be set either in your template or in your StackScript.   

The first is: **"DATABASE_INSTALLATION_TYPE"**  and this must be set to **"DBaaS"** to trigger a managed database deployment

>      DATABASE_INSTALLATION_TYPE="DBaaS"

The second value is : **DATABASE_DBaaS_INSTALLATION_TYPE** and this is quite an involved setting which I will now describe. 

With the **DATABASE_DBaaS_INSTALLATION_TYPE** parameter, you add the configuration qualities of the database you want to be provisioned such as size, region, engine type and so on. If these are all set correctly as part of your build process a managed database according to how you have specified this parameter will be provisioned. If a managed database which satisfies the same provisioning is already running and available then a brand new cluster will not be provisioned but rather the existing cluster will be used. You  can keep your managed database running across multiple deployments of your application, but, you  will have to set **"DB_INSTALL_MODE"** to **"2"** in your template. Please refer to the specification for more information on **"DB_INSTALL_MODE"**. Using DB_INSTALL_MODE you can prevent the database dump from being installed because you already have the database installed from a previous deployment. 

### DIGITAL OCEAN

If you are using Digital Ocean managed databases you can set the following in your template or override for DATABASE_DBaaS_INSTALLATION_TYPE
...
>     DATABASE_DBaaS_INSTALLATION_TYPE="MySQL:DBAAS:mysql:lon1:1:db-s-1vcpu-1gb:8:testdbcluster1:testdb1:testdbuser:nhjfjfubfhbvdhv765g:e265abcb-1295-1d8b-af36-0129f89456c2"
...
An example of provisioning a Postgres cluster can be something like:  

>     DATABASE_DBaaS_INSTALLATION_TYPE="Postgres:DBAAS:pg:lon1:1:db-s-1vcpu-1gb:17:testdbcluster1:testdb1:testdbuser:nhjfjfubfhbvdhv765g:e265abcb-1295-1d8b-af36-0129f89456c2"
...
So, at the time of writing (although there may be nore regions or sizes or database versions by the time you read this) the settings for DATABASE_DBaaS_INSTALLATION_TYPE on Digital Ocean have to be chosen from the following
...
**IMPORTANT:**
  ...
  Once the managed database is provisioned set its "trusted ip" addresses to "10.0.0.0/16" through the GUI system so that it can be accessed through private ip addresses. 
...
So, at the time of writing (although there may be nore regions or sizes or database versions by the time you read this) the settings for DATABASE_DBaaS_INSTALLATION_TYPE on Exoscale have to be chosen from the following
...
So an example of this would be in your template or override to provision a Postgres Cluster

>     DATABASE_DBaaS_INSTALLATION_TYPE="Postgres:DBAAS:postgresql/18:gb-lon:g6-nanode-1:1:test-cluster:testdb1:testdbuser:gdhf76gdfgsh:266632:235421"
...
>     cluster-size="1"
  ...
  db-engine can be **"postgresql/13"** or **"postgresql/14"** or **"postgresql/15"** or **"postgresql/16"** or **"postgresql/17"** or **"postgresql/18"**- at the time of writing you can check what engines are available for you by issuing **"linode-cli databases engines"** command.  
...
>     DATABASE_DBaaS_INSTALLATION_TYPE="MySQL:DBAAS:mysql:8:lhr:vultr-dbaas-hobbyist-cc-1-25-1:testdb:testuser1:testpassword123:TestDatabase:2fb13fd1-3145-3127-7132-13f28f1912c1" 
...
>     DATABASE_DBaaS_INSTALLATION_TYPE="Postgres:DBAAS:pg:14:lhr:vultr-dbaas-hobbyist-cc-1-25-1:testdb:testuser1:testpassword123:TestDatabase:2fb13fd1-3145-3127-7132-13f28f1912c1" 
