...
>     ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/ips/authorised-ips.dat

in the live and put the ip address:

>     0.0.0.0/0

and then create a marker file which tells the system that you intend there to be a firewall update. This is done by creating a file in a specific place that is monitored for and you can do that with the command:

>     /bin/touch ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/FIREWALL-EVENT

in that file and that will open up the ufw or iptables firewall depending on how you are configured. This will mean that the OS firewall will be permissive to SSH connections from any ip address but all other ports will still be blocked by the OS firewall. And then what you can do is you can allow access to specific IP addresses using the cloud native firewall. So, in your cloud native firewall provided by your VPS provider you can allow, for example IP addresses 154.43.3.22/32, 175.33.23.56/32 and 145.32.51.12/32 and that will mean that only those IP addresses will be allowed to connect to your build machine. What that means is that you can easily alter the list of IP addresses which can access your build machine over SSH simply through your VPS provider's firewall GUI system. And so when someone on your team needs a different IP address to be allowed access to the build machine you can easily update the GUI and they will have access to have to contend with updating the OS firewall as well is fine if you are just a single developer but if there is more than one of you with access to the build machine you will likely want to have a simple way to update the allowed IP list just using the GUI system of your cloudhost. Anyway explained below is the full lowdown on how to use the protections that are built in to protect the build machine because it has your most sensitive data on it and is a way in to your system from the outside world. Of course if all of this is too much faff and you aren't so bothered as some people are about having everything tied down you can simply allow connections from any IP addresses over SSH safe in the knowledge that fail2ban is in place and public private encryption keys are in place. If there's more than just you working  with the build machine  then you yourself will have to take steps to provide access for your developer(s) private ssh keys. 
