...
#### Wireguard based authentication operations:
...
What this means is that if you have n authentication servers running they will all write unique files to the datastore which can then be aggregated together into a list of total email addresses from all machines.

- On the reverse proxy machines, the list of aggregated email addresses is collated and processed. The email addresses are collated as follows:
...
And so we now have the aggregation of all email addresses from all authenticator machines on our reverse proxy machine.

- The reverse proxy machine now has to generate the wireguard client configuration and the wireguard server peer to match with the client configuration
The wireguard client interface for our current email address is generated by calling the GenerateClientInterfaceAndPeer.sh script from cron


>     export HOME="${HOME}" && ${HOME}/webserver/configuration/reverseproxy/wire-guard/GenerateClientInterfaceAndPeer.sh

And the server peer to our client's interface is geneated by calling 
...
If the ServerInterface hasn't been previously geneerated and

>     ${HOME}/webserver/configuration/reverseproxy/wire-guard/GenerateServerPeer.sh

If the server interface has already been generated

The Client Interface configuration is then written to the "wire-guard" tagged datastore

>     ${HOME}/services/datastore/operations/SyncToDatastore.sh "wire-guard" "${HOME}/runtime/wire-guard/client/" "distributed"

The authenticator machines then coordinate amongst themselves to decide who needs to send the email with the wireguard configuration to the user and if a machine is elected then it sends the wireguard configuration generated by each reverse proxy as one email to the user by calling:
